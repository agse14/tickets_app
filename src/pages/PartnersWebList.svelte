...
import { crossfade } from 'svelte/transition';
    ...
    const hasAdd = Object.keys(routes).filter(function (propertyName) {
            return propertyName.indexOf("/addpartnersweb") === 0;
        }).length > 0;
    const hasEdit = Object.keys(routes).filter(function (propertyName) {
            return propertyName.indexOf("/editpartnersweb/") === 0;
        }).length > 0;
    ...
    let query = db.collection("partners_web");
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("partners_web").doc(id).update(update);
}
...
<td><a href="/editpartnersweb/{row.id}" use:link>{row.name}</a></td>
<td>{row.lastname != undefined ? row.lastname:''}</td>
...
<td>{row.inbranch?'Si':'No'}</td>
                            ...
                            <td>
                                {#if hasEdit}
                                <a href="/editpartnersweb/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                {/if}
