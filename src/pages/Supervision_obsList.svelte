...
import Supervision_obsEdit from "./Supervision_obsEdit.svelte";

import {branches} from "../controls/branches.js"
import BranchesSearchList from "../controls/BranchesSearchList.svelte"
import { status as supervision_obs_status} from "../controls/supervision_obs.js"
...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.supervision_obs){
    ...
    function loadData(){
        ...
        datatable = jQuery('#supervision_obs-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Tarea', '25 Tarea', '50 Tarea', 'Todo' ]
            ],
            data: supervision_obs,
            ...
            columns: [
                { data: 'added', name:'Fecha', render: function (data, type, row, meta) {
                    ...
                    return type == 'sort'? date.toISODate():html('<a href="#/editsupervision_obs/'+id+'">'+date.toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY)+'</a>');
                }, },
{ data: 'branchesName', name:'Estancia' },
{ data: 'name', name:'A revisar', render: function (data, type, row, meta) {
					var id = "";
                    if(row != undefined) id = row.id;
					if(type.cells != undefined){
						id = type.cells[type.cells.length-1].data;
					}
                    return html('<a href="#/editsupervision_obs/'+id+'">'+data+'</a>');
                }, },
{ data: 'statusName', name:'Estatus' },
{ data: 'priorityName', name:'Prioridad' },
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					let buttons = "";
                    if (row.status == 1 && ($permissions.operations || $permissions.admin)) {
                    buttons += "<button onclick=\"markCompleted('" +
                                data +
                                '\')" class="btn btn-success waves-effect waves-light"><i class="bx bx-check font-size-16 align-middle"></i>Completo</button>';
                    }
					if(hasEdit){
						if(modal){
                            if($permissions.operations || $permissions.admin){
							...
							buttons += '<a href="#/editsupervision_obs/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
                    if($permissions.operations || $permissions.admin){
            ...
            datatable.buttons().container()
                .appendTo('#supervision_obs-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        let query = db.collection("supervision_obs");
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                tempsupervision_obs.did = doc.id;
                if(tempsupervision_obs.priority == undefined){
                    tempsupervision_obs.priority = 0;
                    tempsupervision_obs.priorityName = "Baja";
                }else{
                    tempsupervision_obs.priorityName = (tempsupervision_obs.priority == 1) ? "Media" : "Alta";
                }
                
				tempsupervision_obs.branchesName = "";
				$branches.forEach(element => {
                if(element.id == tempsupervision_obs.branches){
                  tempsupervision_obs.branchesName = element.name;
                }
              });
tempsupervision_obs.statusName = (tempsupervision_obs.status != undefined && supervision_obs_status[tempsupervision_obs.status] != undefined) ? supervision_obs_status[tempsupervision_obs.status] : '';
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(supervision_obs).draw(false);
            }
            // if(datatable != undefined){
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("supervision_obs").doc(id).delete();
...
function markCompleted(id) {
        Swal.fire({
            title: "¿Esta seguro?",
            text: "La tearea ya fue realizada?",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#34c38f",
            cancelButtonColor: "#f46a6a",
            confirmButtonText: "Si, realizada!",
        }).then(function (result) {
            if (result.value) {
                var update = {};
                update["status"] = 2;
                update["completed"] = firebase.firestore.FieldValue.serverTimestamp();
                update["completedBy"] = $fbuser.uid;
                db.collection("supervision_obs").doc(id).update(update);
            }
        });
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Tarea</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <!-- <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Tareas de supervisión</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addsupervision_obs/'+filterName+'/'+filterValue);
                            }else{
                                push('/addsupervision_obs');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Tareas de supervisión</button>
                        {/if}
                    </div> -->
                  </div>
                {/if}
                
                <table id="supervision_obs-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Fecha</th>
<th>Estancia</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Tareas de supervisión</h5>
                            ...
                            {#if showModal}
                                <Supervision_obsEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each supervision_obs as row (row.id)}
                        <tr in:receive="{{key: row.id}}"
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td><h5>{row.branchesName}</h5></td>
<td><a href="/editsupervision_obs/{row.id}" use:link>{row.name}</a></td>
<td><Supervision_obsStatusListSelect value={row.status} inlist={true} /></td>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editsupervision_obs/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
