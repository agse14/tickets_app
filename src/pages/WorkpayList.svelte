...
import { crossfade } from 'svelte/transition';
    ...
    const hasAdd = Object.keys(routes).filter(function (propertyName) {
            return propertyName.indexOf("/addworkpay") === 0;
        }).length > 0;
    const hasEdit = Object.keys(routes).filter(function (propertyName) {
            return propertyName.indexOf("/editworkpay/") === 0;
        }).length > 0;
    ...
    function loadData(){
        datatable = jQuery('#workpay-dt').DataTable({
                lengthChange: false,
            ...
            datatable.buttons().container()
                .appendTo('#workpay-dt_wrapper .col-md-6:eq(0)');
    }
    ...
    unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
            ...
            const tempworkpay = doc.data();
            tempworkpay.id = doc.id;
            tmp.push(tempworkpay);
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("workpay").doc(id).delete();
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("workpay").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Asistencias" base="Inventario" />

<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Administración Asistencias</h4>
                 {#if filterName != undefined && filterValue != undefined}
                    <FilterName node={filterName} value={filterValue} />
                 {/if}
                 <div class="row"><div class="col">
                    <BranchesSearchList value={values.bid} inlist={false} />
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addworkpay/'+filterName+'/'+filterValue);
                            }else{
                                push('/addworkpay');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Asistencia</button>
                    </div>
                  </div>
                {/if}
                <table id="workpay-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Sucursal</th>
<th>Día</th>
...
<th>Retardo</th>
                    ...
                    <tbody>
                        ...
                        {#each workpay as row (row.id)}
                        <tr in:receive="{{key: row.id}}"
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td><BranchesSearchList value={row.branches} inlist={true} /></td>
<td>{row.day != undefined ? row.day:''}</td>
<td>{row.hour != undefined ? row.hour:''}</td>
<td>$ {row.pay != undefined ? row.pay:''}</td>
<td><WorkersSearchList value={row.workers} inlist={true} /></td>
<td><PositionsSearchList value={row.positions} inlist={true} /></td>
<td>{row.late?'Si':'No'}</td>

                            <td>
                                {#if hasEdit}
                                <a href="/editworkpay/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                {/if}
