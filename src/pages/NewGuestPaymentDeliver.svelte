...
import PatientSearchList from "../controls/PatientSearchList.svelte"
  ...
  */
  ...
  unsubscribe = db.collection("recipt_payments")
    .where("transit", "==", 0).where("paytype", "in", [1,2])
    .orderBy("added", "desc")
  ...
  async function recipt(payment) {
    // console.log("recipt ", payment, patientdata);

    // db.collection("recipt_payments").doc(payment.id).update({transit:1});
    // Swal input password
    let total = payment.total.toLocaleString("es-MX");
    let res = await Swal.fire(
      {
        title: "¿Esta seguro?",
        text: "Desea marcar el pago de $"+total+" como enviada a oficinas? Favor de confirmar con la contraseña de recepción",
        type: "warning",
        input: "password",
        showCancelButton: true,
        confirmButtonColor: "#34c38f",
        cancelButtonColor: "#f46a6a",
        confirmButtonText: "Si, enviada!",
      }
    );
    console.log(res);
    if(res.value == "3476"){
      // console.log(res.value);
      db.collection("recipt_payments").doc(payment.id).update({transit:2,colecteddate:DateTime.local().toISODate()});
    }
    ...
    //             if (result.value) {
    //               db.collection("recipt_payments").doc(payment.id).update({transit:2,transitdate:DateTime.local().toISODate()});
                  ...
                  {#each guestpayments as pagos}
                    <tr>
                      ...
                      <td>
                        ...
                        {#if $permissions.admin || $permissions.admin_branch || $permissions.collect_payments}
                          <button
                          type="button"
                          on:click={() => recipt(pagos)}
                          class="btn btn-success waves-effect waves-light"
                          disabled={loading}>
                            <i class="bx bx-send font-size-16 align-middle"/>
                          </button>
                        {/if}
                      </td>
                    </tr>
