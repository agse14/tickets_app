...
import PositionsEdit from "./PositionsEdit.svelte";
...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.positions){
    ...
    function loadData(){
        ...
        datatable = jQuery('#positions-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Puestos', '25 Puestos', '50 Puestos', 'Todo' ]
            ],
            data: positions,
            columns: [
                { data: 'name', name:'Puesto', render: function (data, type, row, meta) {
					var id = "";
                    if(row != undefined) id = row.id;
					if(type.cells != undefined){
						id = type.cells[type.cells.length-1].data;
					}
                    return html('<a href="#/editpositions/'+id+'">'+data+'</a>');
                }, },
{ data: 'base_pay', render: function (data, type, row, meta) {
...
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editpositions/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#positions-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        let query = db.collection("positions");
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(positions).draw(false);
            }
            // if(datatable != undefined){
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("positions").doc(id).delete();
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("positions").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Puestos" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Puestos</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Puesto</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addpositions/'+filterName+'/'+filterValue);
                            }else{
                                push('/addpositions');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Puesto</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="positions-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Puesto</th>
<th>Sueldo base</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Puesto</h5>
                            ...
                            {#if showModal}
                                <PositionsEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each positions as row (row.id)}
                        ...
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td><a href="/editpositions/{row.id}" use:link>{row.name}</a></td>
{ data: 'base_pay', render: function (data, type, row, meta) {
                    return '$ '+(data != undefined?data:'');
                }, },
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editpositions/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
