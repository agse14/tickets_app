...
import Supervision_tagsEdit from "./Supervision_tagsEdit.svelte";

import {branches} from "../controls/branches"
import ImageField from "../controls//ImageField.svelte";
import { status as supervision_tags_status} from "../controls/supervision_tags.js"
...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.supervision_tags){
    ...
    function loadData(){
        ...
        datatable = jQuery('#supervision_tags-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Puntos de supervisión', '25 Puntos de supervisión', '50 Puntos de supervisión', 'Todo' ]
            ],
            data: supervision_tags,
            createdRow: function (row, data, index) {
                
            },
            columns: [
                { data: 'statusName', name:'Estatus',width:"auto" },
{ data: 'name', name:'Identificador',width:"auto", render: function (data, type, row, meta) {
					var id = "";
                    ...
                    return html('<a href="#/editsupervision_tags/'+id+'">'+data+'</a>');
                }, },
{ data: 'branchesName', name:'Estancia',width:"auto" },
{ data: 'location', render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                }, },
{ data: 'last_scan', name:'Última revisión',width:"auto", render: function (data, type, row, meta) {
							return html(timeToAgo(data)+'<br /><small>'+timeFormated(data)+'</small>');
						}, },
{ data: 'usersName', name:'Revisado Por',width:"auto" },
{ data: 'id', name:'ID',width:"auto", render: function (data, type, row, meta) {
                    return html(data);
                }, },
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editsupervision_tags/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#supervision_tags-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        let query = db.collection("supervision_tags");
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                tempsupervision_tags.did = doc.id;
                tempsupervision_tags.statusName = (tempsupervision_tags.status != undefined && supervision_tags_status[tempsupervision_tags.status] != undefined) ? supervision_tags_status[tempsupervision_tags.status] : '';
			
				tempsupervision_tags.branchesName = "";
				$branches.forEach(elbranches => {
                if(elbranches.id == tempsupervision_tags.branches){
                  tempsupervision_tags.branchesName = elbranches.name;
                }
              });
			
				tempsupervision_tags.usersName = "";
				$users.forEach(elusers => {
                if(elusers.id == tempsupervision_tags.users){
                  tempsupervision_tags.usersName = elusers.name;
                }
              });
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(supervision_tags).draw(false);
            }
            // if(datatable != undefined){
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("supervision_tags").doc(id).delete();
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("supervision_tags").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Puntos de supervisión" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Puntos de supervisión</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Punto de supervisión</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addsupervision_tags/'+filterName+'/'+filterValue);
                            }else{
                                push('/addsupervision_tags');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Punto de supervisión</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="supervision_tags-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Estatus</th>
<th>Identificador</th>
<th>Estancia</th>
<th>Ubicación en estancia</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Punto de supervisión</h5>
                            ...
                            {#if showModal}
                                <Supervision_tagsEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each supervision_tags as row (row.id)}
                        <tr in:receive="{{key: row.id}}"
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td><Supervision_tagsStatusListSelect value={row.status} inlist={true} /></td>
<td><a href="/editsupervision_tags/{row.id}" use:link>{row.name}</a></td>
<td><h5>{row.branchesName}</h5></td>
{ data: 'location', render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                }, },
<td>{timeToAgo(row.last_scan)}<br /><small>{timeFormated(row.last_scan)}</small></td>
<td><h5>{row.usersName}</h5></td>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editsupervision_tags/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
