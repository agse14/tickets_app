...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.users){
    ...
    function loadData(){
        ...
        datatable = jQuery('#users-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Usuarios', '25 Usuarios', '50 Usuarios', 'Todo' ]
            ],
            data: users,
            ...
            columns: [
                { data: 'name', name:'Nombre',width:"auto", render: function (data, type, row, meta) {
					var id = "";
                    ...
                    return html('<a href="#/editusers/'+id+'">'+data+'</a>');
                }, },
...
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editusers/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#users-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        ...
        unsubscribe = query.where("system", "==", 1).
                        orderBy('added','desc').
            ...
            querySnapshot.forEach((doc) => {
                ...
                tempusers.id = doc.id;
                tempusers.did = doc.id;
                			
				tempusers.branchesName = "";
				$branches.forEach(elbranches => {
                if(elbranches.id == tempusers.branches){
                  tempusers.branchesName = elbranches.name;
                }
              });
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(users).draw(false);
            }
            // if(datatable != undefined){
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("users").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Usuarios" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Usuarios</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Usuario</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addusers/'+filterName+'/'+filterValue);
                            }else{
                                push('/addusers');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Usuario</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="users-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Nombre</th>
<th>Email</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Usuario</h5>
                            ...
                            {#if showModal}
                                <UsersEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each users as row (row.id)}
                        ...
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td><a href="/editusers/{row.id}" use:link>{row.name}</a></td>
{ data: 'email', render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                }, },
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editusers/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
