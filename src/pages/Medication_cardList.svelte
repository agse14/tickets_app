...
import Medication_cardEdit from "./Medication_cardEdit.svelte";
...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.medication_card){
    ...
    function loadData(){
        ...
        datatable = jQuery('#medication_card-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Cartas de Medicación', '25 Cartas de Medicación', '50 Cartas de Medicación', 'Todo' ]
            ],
            data: medication_card,
            columns: [
                { data: 'name', name:'Nombre Residente: ', render: function (data, type, row, meta) {
					var id = "";
                    var patientName = "";
                    if(row != undefined) 
                        id = row.id;
                        patientName = row.nameName;
                        console.log(row);
					if(type.cells != undefined){
						id = type.cells[type.cells.length-1].data;
					}
                    return html('<a href="#/editmedication_card/'+id+'">'+patientName+'</a>');
                }, },
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editmedication_card/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#medication_card-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        let query = db.collection("medication_card");
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                const tempmedication_card = doc.data();
                tempmedication_card.id = doc.id;
                tempmedication_card.did = doc.id;
                			
				tempmedication_card.nameName = "";
				$patients.forEach(element => {
                if(element.id == tempmedication_card.name){
                  tempmedication_card.nameName = element.name;
                }
              });

                tmp.push(tempmedication_card);
                
                
            });
            medication_card = tmp;
            // for (let i = 0; i < medication_card.length; i++) {
            //     if (medication_card[i].name) {
            //         $patients.forEach(element => {
            //             if(element.id == medication_card[i].name){
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(medication_card).draw(false);
            }
            // if(datatable != undefined){
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("medication_card").doc(id).delete();
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("medication_card").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Cartas de Medicación" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Cartas de Medicación</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Carta</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addmedication_card/'+filterName+'/'+filterValue);
                            }else{
                                push('/addmedication_card');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Carta</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="medication_card-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content" style="overflow-y: scroll;">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Carta</h5>
                            ...
                            {#if showModal}
                                <Medication_cardEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each medication_card as row (row.id)}
                        <tr in:receive="{{key: row.id}}"
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td><a href="/editmedication_card/{row.id}" use:link>{row.name}</a></td>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editmedication_card/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
