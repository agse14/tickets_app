...
import CardactivitiesEdit from "./CardactivitiesEdit.svelte";

import {activities} from "../controls/activities.js"
import ActivitySearchList from "../controls/ActivitySearchList.svelte"
...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.cardActivities){
    ...
    function loadData(){
        ...
        datatable = jQuery('#cardActivities-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Tarjeta de actividades', '25 Tarjeta de actividades', '50 Tarjeta de actividades', 'Todo' ]
            ],
            data: cardActivities,
            ...
            columns: [
                { data: 'dayId', name:'Fecha' },
{ data: 'patientName', name:'Huesped' },
...
{ data: 'name', name:'Nombre', render: function (data, type, row, meta) {
                    ...
                    return html('<a href="#/editcardActivities/'+id+'">'+data+'</a>');
                }, },
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editcardActivities/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#cardActivities-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                tempcardActivities.did = doc.id;
                			
				tempcardActivities.patientName = "";
				$patients.forEach(element => {
                if(element.id == tempcardActivities.patient){
                  tempcardActivities.patientName = element.name;
                }
              });
			
				tempcardActivities.activityName = "";
				$activities.forEach(element => {
                if(element.id == tempcardActivities.activity){
                  tempcardActivities.activityName = element.name;
                }
              });
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(cardActivities).draw(false);
            }
            // if(datatable != undefined){
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("cardActivities").doc(id).delete();
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("cardActivities").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Tarjeta de actividades" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Tarjeta de actividades</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Tarjetas de Actividades</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addcardActivities/'+filterName+'/'+filterValue);
                            }else{
                                push('/addcardActivities');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Tarjetas de Actividades</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="cardActivities-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Fecha</th>
<th>Huesped</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Tarjetas de Actividades</h5>
                            ...
                            {#if showModal}
                                <CardactivitiesEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each cardActivities as row (row.id)}
                        <tr in:receive="{{key: row.id}}"
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td>{row.dayId}</td>
<td><h5>{row.patientName}</h5></td>
{ data: 'time', render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                }, },
<td><a href="/editcardActivities/{row.id}" use:link>{row.name}</a></td>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editcardActivities/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
