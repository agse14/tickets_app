...
import { crossfade } from 'svelte/transition';
    ...
    const hasAdd = Object.keys(routes).filter(function (propertyName) {
            return propertyName.indexOf("/addclinic_history") === 0;
        }).length > 0;
    const hasEdit = Object.keys(routes).filter(function (propertyName) {
            return propertyName.indexOf("/editclinic_history/") === 0;
        }).length > 0;
    ...
    function loadData(){
        datatable = jQuery('#clinic_history-dt').DataTable({
                lengthChange: false,
            ...
            datatable.buttons().container()
                .appendTo('#clinic_history-dt_wrapper .col-md-6:eq(0)');
    }
	
    let query = db.collection("clinic_history");
    ...
    unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
            ...
            const tempclinic_history = doc.data();
            tempclinic_history.id = doc.id;
            tmp.push(tempclinic_history);
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("clinic_history").doc(id).delete();
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("clinic_history").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Historiales Clinicos" base="Inventario" />

<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Administración Historiales Clinicos</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addclinic_history/'+filterName+'/'+filterValue);
                            }else{
                                push('/addclinic_history');
                            }
                ...
                {/if}
                <table id="clinic_history-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        {#each clinic_history as row (row.id)}
                        ...
                        animate:flip>
                            ...
                            <td>
                                {#if hasEdit}
                                <a href="/editclinic_history/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                {/if}
