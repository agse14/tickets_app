...
import Drug_inventoryEdit from "./Drug_inventoryEdit.svelte";

import {prescriptions} from "../controls/prescriptions.js"
import PrescriptionsSearchList from "../controls/PrescriptionsSearchList.svelte"
...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.drug_inventory){
    ...
    function loadData(){
        ...
        datatable = jQuery('#drug_inventory-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Ingreso de inventario', '25 Ingreso de inventario', '50 Ingreso de inventario', 'Todo' ]
            ],
            data: drug_inventory,
            ...
            columns: [
                { data: 'prescriptionsName', name:'Medicamento' },
{ data: 'dosis', render: function (data, type, row, meta) {
...
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editdrug_inventory/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#drug_inventory-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        let query = db.collection("drug_inventory");
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                tempdrug_inventory.did = doc.id;
                			
				tempdrug_inventory.prescriptionsName = "";
				$prescriptions.forEach(element => {
                if(element.id == tempdrug_inventory.prescriptions){
                  tempdrug_inventory.prescriptionsName = element.name;
                }
              });
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(drug_inventory).draw(false);
            }
            // if(datatable != undefined){
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("drug_inventory").doc(id).delete();
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("drug_inventory").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Ingreso de inventario" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Ingreso de inventario</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Ingresos</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/adddrug_inventory/'+filterName+'/'+filterValue);
                            }else{
                                push('/adddrug_inventory');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Ingresos</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="drug_inventory-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Medicamento</th>
<th>Dosis a agregar</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Ingresos</h5>
                            ...
                            {#if showModal}
                                <Drug_inventoryEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each drug_inventory as row (row.id)}
                        <tr in:receive="{{key: row.id}}"
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td><h5>{row.prescriptionsName}</h5></td>
{ data: 'dosis', render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                }, },
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editdrug_inventory/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
