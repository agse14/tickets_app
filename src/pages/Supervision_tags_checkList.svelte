...
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
	...
	function loadFirebaseData(){
        let query = db.collection("supervision_tags_check");
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                if(elsupervision_tags.id == tempsupervision_tags_check.supervision_tags){
                  tempsupervision_tags_check.supervision_tagsName = elsupervision_tags.name;
                }
              });
			
				tempsupervision_tags_check.branchesName = "";
				$branches.forEach(elbranches => {
                if(elbranches.id == tempsupervision_tags_check.branches){
                  tempsupervision_tags_check.branchesName = elbranches.name;
                }
              });
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Revisión</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addsupervision_tags_check/'+filterName+'/'+filterValue);
                            }else{
                                push('/addsupervision_tags_check');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Revisión</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="supervision_tags_check-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Fecha</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Revisión</h5>
                            ...
                            {#if showModal}
                                <Supervision_tags_checkEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
