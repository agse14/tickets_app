...
import VacationsEdit from "./VacationsEdit.svelte";
import VacationsPrint from "./VacationsPrint.svelte";
...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.vacations){
    ...
    function loadData(){
        ...
        datatable = jQuery('#vacations-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Vacaciones', '25 Vacaciones', '50 Vacaciones', 'Todo' ]
            ],
            data: vacations,
            columns: [
                { data: 'start', name:'Fecha de inicio', render: function (data, type, row, meta) {
							return html(row.workersName+'<br />'+timeToAgo(data)+'<br /><small>'+timeFormated(data)+'</small><br />');
						}, },
                { data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					let buttons = "";
					if(hasEdit){
						if(modal){
							buttons += '<button type="button" class="btn btn-light waves-effect" onclick="setEditId(\''+data+'\');"><i class="bx bx-edit-alt font-size-16 align-middle"></i></button><button type="button" class="btn btn-light waves-effect" onclick="setPrintId(\''+data+'\');"><i class="bx bx-printer font-size-16 align-middle"></i></button>';
						}else{
							buttons += '<a href="#/editvacations/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#vacations-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                const tempvacations = doc.data();
                tempvacations.id = doc.id;
                tempvacations.did = doc.id;
                			
				tempvacations.workersName = "";
				$workers.forEach(element => {
                if(element.id == tempvacations.workers){
                  tempvacations.workersName = element.name;
                }
              });

                tmp.push(tempvacations);
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(vacations).draw(false);
            }
            // if(datatable != undefined){
...
function setPrintId(eId) {
    editData = {field:filterName, value:filterValue,bid:eId};
    showModal = true;
    jQuery('#printModalScrollable').modal('show');
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("vacations").doc(id).update(update);
}
window.setEditId = setEditId;
window.setPrintId = setPrintId;
...
<div class="page-content">
    ...
    <TitleBar title="Vacaciones" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Vacaciones</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Dias de vacación</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addvacations/'+filterName+'/'+filterValue);
                            }else{
                                push('/addvacations');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Dias de vacación</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="vacations-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content" style="overflow-y: scroll;">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Dias de vacación</h5>
                            ...
                            {#if showModal}
                                <VacationsEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
                ...
                <div class="modal fade" id="printModalScrollable" tabindex="-1" role="dialog" aria-labelledby="printModalScrollableTitle" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content" style="overflow-y: scroll;">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="printModalScrollableTitle">Impresión</h5>
                                <button type="button" class="close" on:click={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#printModalScrollable').modal('hide');
                                    showModal = false;
                                }} aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            ...
                            {#if showModal}
                                <VacationsPrint params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#printModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
                        ...
                        animate:flip>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editvacations/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
