<script>
      ...
      .onSnapshot((result) => {
        ...
        for (let r = 0; r < rids.length; r++) {
          ...
          if (recibos[rid].paymentperiod != "Especial") {
            ...
            // ✅ Corregido: Comparación numérica en lugar de string
            if (recibos[rid].total >= 0) {
  ...
  // Variables reactivas para filtrar datos por tipo de pago
  $: filteredRecibosOrder = Object.values(recibos_order).filter(worker => {
    return filter === "All" || worker.paymentperiod === filter;
  });

  // Variable reactiva para calcular totales por concepto filtrados
  $: filteredConceptTotals = (() => {
    const conceptTotals = {};
    
    // Solo procesar workers que coincidan con el filtro de tipo de pago
    filteredRecibosOrder.forEach(worker => {
      if (!worker.uid) return;
      
      // ✅ Agregar sueldo fijo base para workers con payroll "Fijo" (como en loadFireStore)
      if (worker.paymentperiod !== "Especial" && worker.payroll === "Fijo" && worker.payamount) {
        const sueldoFijoAmount = worker.payamount / 2;
        const conceptName = `${worker.branchName} - Sueldo ${worker.positionsName || ""}`;
        const branchName = worker.branchName;
        
        // Inicializar concepto de sueldo fijo
        if (!conceptTotals[conceptName]) {
          conceptTotals[conceptName] = { 
            totalAmount: 0, 
            totalUnits: 0, 
            branches: {} 
          };
        ...
        // Inicializar branch si no existe
        if (!conceptTotals[conceptName].branches[branchName]) {
          conceptTotals[conceptName].branches[branchName] = { 
            totalAmount: 0, 
            totalUnits: 0 
          };
        }
        
        // Sumar sueldo fijo base
        conceptTotals[conceptName].totalAmount += sueldoFijoAmount;
        conceptTotals[conceptName].totalUnits += 1; // 1 unidad por el sueldo base
        conceptTotals[conceptName].branches[branchName].totalAmount += sueldoFijoAmount;
        conceptTotals[conceptName].branches[branchName].totalUnits += 1;
      }
      
      // ✅ Procesar CONCEPTS (guardias principales)
      if (worker.concepts && worker.concepts.length > 0) {
        worker.concepts.forEach(concept => {
          if (!concept.uid) return;
          
          const conceptName = concept.detailName || 
            ("Guardia" + (concept.positionsName ? ` - ${concept.positionsName}` : ""));
          const branchName = concept.branchName;
          
          // Inicializar concepto si no existe
          if (!conceptTotals[conceptName]) {
            conceptTotals[conceptName] = { 
              totalAmount: 0, 
              totalUnits: 0, 
          ...
          // Sumar totales de concepts (guardias)
          conceptTotals[conceptName].totalAmount += concept.amount * concept.units;
          conceptTotals[conceptName].totalUnits += concept.units;
          conceptTotals[conceptName].branches[branchName].totalAmount += concept.amount * concept.units;
          conceptTotals[conceptName].branches[branchName].totalUnits += concept.units;
        });
      }
      
      // ✅ Procesar PATIENTLOG y EXTRA (complementos)
      [...(worker.patientlog || []), ...(worker.extra || [])].forEach(record => {
        if (!record.uid || record.createdBy === "pending") return;
        
        const conceptName = record.detailName || 
          ("Guardia" + (record.positionsName ? ` - ${record.positionsName}` : ""));
        ...
        // Sumar totales de patientlog/extra
        conceptTotals[conceptName].totalAmount += record.amount * record.units;
        conceptTotals[conceptName].totalUnits += record.units;
        conceptTotals[conceptName].branches[branchName].totalAmount += record.amount * record.units;
        conceptTotals[conceptName].branches[branchName].totalUnits += record.units;
      });
    });
    
    console.log('📊 Conceptos calculados:', Object.keys(conceptTotals));
    console.log('📊 Filtro aplicado:', filter);
    console.log('📊 Workers filtrados:', filteredRecibosOrder.length);
    console.log('📊 Total con sueldo fijo:', Object.values(conceptTotals).reduce((total, concept) => total + concept.totalAmount, 0));
    
    return conceptTotals;
  })();

  // Total calculado para conceptos filtrados
  $: filteredConceptTotal = Object.values(filteredConceptTotals)
    .reduce((total, concept) => total + concept.totalAmount, 0);
    ...
    <div class="card">
      <div class="card-body">
        ...
        {:else if report == "2"}
          ...
          {:else}
            {#if filterBranch == "All" || Object.keys(filteredConceptTotals).length > 0}
              <table
                id="table"
                class="table table-striped print-no-break mb-4 mt-4"
              >
                <tbody>
                  ...
                  {#each Object.keys(filteredConceptTotals) as concept}
                    {#if filterBranch === "All" || filteredConceptTotals[concept]?.branches?.[filterBranch]}
                      <tr>
                        ...
                        <td style="text-align: left;">
                          {#if filterBranch === "All"}
                            {Intl.NumberFormat("es-MX", {
                              style: "currency",
                              currency: "MXN",
                            }).format(filteredConceptTotals[concept].totalAmount)}
                          {:else}
                            {Intl.NumberFormat("es-MX", {
                              style: "currency",
                              currency: "MXN",
                            }).format(filteredConceptTotals[concept]?.branches?.[filterBranch]?.totalAmount || 0)}
                          {/if}
                        </td>
                        <td style="text-align: left;">
                          {#if filterBranch === "All"}
                            {filteredConceptTotals[concept].totalUnits}
                          {:else}
                            {filteredConceptTotals[concept]?.branches?.[filterBranch]?.totalUnits || 0}
                          {/if}
                        </td>
                      </tr>
                    {/if}
                      ...
                      <b>
                        {#if filterBranch === "All"}
                          {Intl.NumberFormat("es-MX", {
                            style: "currency",
                            currency: "MXN"
                          }).format(filteredConceptTotal)}
                        {:else}
                          {Intl.NumberFormat("es-MX", {
                            style: "currency",
                            currency: "MXN"
                          }).format(
                            Object.values(filteredConceptTotals)
                              .reduce((total, concept) => 
                                total + (concept?.branches?.[filterBranch]?.totalAmount || 0), 0)
                          )}
                        {/if}
                      </b>         
                    </td>
