...
import { crossfade } from 'svelte/transition';
    ...
    $: filteredActivities = typeFilter === null
        ? activities
        : activities.filter(a => a.type === typeFilter);

    $: if (datatable) {
        datatable.clear();
        datatable.rows.add(filteredActivities).draw(false);
    }
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.activities){
    ...
    function loadData(){
        ...
        datatable = jQuery('#activities-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Programación de actividades', '25 Programación de actividades', '50 Programación de actividades', 'Todo' ]
            ],
            ...
            columns: [
                { data: 'start', name:'Fecha de inicio', render: function (data, type, row, meta) {
					return html(row.patientName+'<br />'+row.name+'<br /><small>Finaliza '+timeToAgo(row.endDate)+'</small><br />');
				    }, 
                },
                {data:'branchName', name:"Estancia"},
                { data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					// 	}else{
					// 		buttons += '<a href="#/editactivities/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
					// 	}
					// }
            ...
            datatable.buttons().container()
                .appendTo('#activities-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                tempactivities.did = doc.id;
                			
				tempactivities.patientName = "";
				$patients.forEach(element => {
                if(element.id == tempactivities.patient){
                  tempactivities.patientName = element.name;
                }
              });

              $branches.forEach((elbranches) => {
                        if (elbranches.id == tempactivities.branches) {
                            tempactivities.branchName = elbranches.name;
                        }
                    });
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(activities).draw(false);
            }
            // if(datatable != undefined){
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("activities").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Programación de actividades" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Programación de actividades</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Actividades programadas</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addactivities/'+filterName+'/'+filterValue);
                            }else{
                                push('/addactivities');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Actividades programadas</button>
                        {/if}
                    </div>
                </div>
                {/if}
                <div class="btn-group mb-3" role="group">
                    <button
                        class="btn {typeFilter === null ? 'btn-primary' : 'btn-outline-primary'}"
                        on:click={() => typeFilter = null}
                    >
                        Todas
                    </button>
                    <button
                        class="btn {typeFilter === 0 ? 'btn-info' : 'btn-outline-info'}"
                        on:click={() => typeFilter = 0}
                    >
                        Paciente
                    </button>
                    <button
                        class="btn {typeFilter === 1 ? 'btn-info' : 'btn-outline-info'}"
                        on:click={() => typeFilter = 1}
                    >
                        Limpieza    
                    </button>
                    <button
                        class="btn {typeFilter === 2 ? 'btn-info' : 'btn-outline-info'}"
                        on:click={() => typeFilter = 2}
                    >
                        Lavanderia
                    </button>
                </div>
                
                <table id="activities-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Actividades programadas</h5>
                            ...
                            {#if showModal}
                                <ActivitiesEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
                        ...
                        animate:flip>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editactivities/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
