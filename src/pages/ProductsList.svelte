...
import ProductsEdit from "./ProductsEdit.svelte";
...
import { crossfade } from 'svelte/transition';
    ...
    const hasAdd = Object.keys(routes).filter(function (propertyName) {
            return propertyName.indexOf("/addproducts") === 0;
        }).length > 0;
    const hasEdit = Object.keys(routes).filter(function (propertyName) {
            return propertyName.indexOf("/editproducts/") === 0;
        }).length > 0;
    ...
    function loadData(){
        ...
        datatable = jQuery('#products-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
        ...
        columns: [
            ...
            datatable.buttons().container()
                .appendTo('#products-dt_wrapper .col-md-6:eq(0)');
    }
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Producto</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addproducts/'+filterName+'/'+filterValue);
                            }else{
                                push('/addproducts');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Producto</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="products-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Producto</th>
<th>Unidades</th>
...
<th>Categoría</th>
                    ...
                    <tbody>
                        ...
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td><a href="/editproducts/{row.id}" use:link>{row.name}</a></td>
<td><ProductsUnitListSelect value={row.unit} inlist={true} /></td>
<td><ProductsStoreListSelect value={row.store} inlist={true} /></td>
<td><ProductsTypeListSelect value={row.type} inlist={true} /></td>
<td><ProductsCategoryListSelect value={row.category} inlist={true} /></td>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editproducts/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Producto</h5>
                            ...
                            {#if showModal}
                                <ProductsEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
