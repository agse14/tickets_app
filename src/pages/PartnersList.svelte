...
import { crossfade } from 'svelte/transition';
    ...
    function loadData(){
        ...
        datatable = jQuery('#partners-dt').DataTable({
            ...
            order: [ [1,'asc'] ],
            columns: [
                { data: 'active_service', name:'Activo', render: function (data, type, row, meta) {
                    return data === true
                        ? 'Si'
                        : 'No';
                    }, 
                },
                { data: 'folio', name : 'Folio', render: function (data, type, row, meta) {
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        // Verificación de cada valor antes de concatenarlos
                        var fullName = (row.name || '') + ' ' + (row.lastname || '') + ' ' + (row.lastname_2 || '');
                        return html('<a href="#/editpartners/' + id + '">' + fullName.trim() + '</a>');
                    },
                },
                ...
                { data: 'pay', visible: false, render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                    }, 
                },
                { data: 'branchesName', visible: false, render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                    }, 
                },
                { data: 'active_date', visible: false, render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                    }, 
                },
                { data: 'active_date_off', visible: false, render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                    }, 
                },
                { data: 'active', name:'Activo en Medi', render: function (data, type, row, meta) {
                    return data === true
                        ? 'Si'
                        : 'No';
                    }, 
                },
                { data: 'inbranch', name:'En Estancia', render: function (data, type, row, meta) {
                    return data === true
                        ? 'Si'
                        : 'No';
                    }, 
                ...
                { data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editpartners/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Empleados</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Empleado</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addpartners/'+filterName+'/'+filterValue);
                            }else{
                                push('/addpartners');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Empleado</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="partners-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Activo</th>
                            <th>Folio</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Empleado</h5>
                            ...
                            {#if showModal}
                                <PartnersEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each partners as row (row.id)}
                        <tr in:receive="{{key: row.id}}"
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td>{row.active_service?'Si':'No'}</td>
<td><a href="/editpartners/{row.id}" use:link>{row.name}</a></td>
{ data: 'lastname', render: function (data, type, row, meta) {
                    return ''+(data != undefined?data:'');
                }, },
{ data: 'email', render: function (data, type, row, meta) {
...
<td>{row.inbranch?'Si':'No'}</td>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editpartners/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
