...
import { crossfade } from 'svelte/transition';
    ...
    function loadBranch(){
        ...
        for (const sc of supervision_tags_check) {
            if(sc.branch == filterValue){
                
                if(sc.dayId != undefined){

                    const date = DateTime.fromISO(sc.dayId).setLocale("es-mx");
                    const added = DateTime.fromJSDate(sc.added.toDate()).setLocale("es-mx");
                    if (added.startOf('day') > date.startOf('day')) {
                        continue;
                    }
                    sc.addedTime = date;
                    let isoDate =sc.dayId;
                    if(checksDates[isoDate] == undefined){
                        checksDates[isoDate] = {date:date,checks:0, total:0, review:[]};
                    }
                    if(sc.checkActivity == true){
                        checksDates[isoDate].checks++;
                    }
                    checksDates[isoDate].total++;
                    checksDates[isoDate].review.push(sc);
        ...
        //order by .patient
        // checksDates["2024-07-07"]?.review?.sort((a,b)=>{
    ...
    function loadData(){
        ...
        datatable = jQuery('#supervision_day_check-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Historial de revisiones', '25 Historial de revisiones', '50 Historial de revisiones', 'Todo' ]
            ],
            ...
            columns: [
                { data: 'added', name:'Fecha',width:"auto", render: function (data, type, row, meta) {
                    ...
                    return type == 'sort'? date.toISODate():html('<a href="#/editsupervision_tags_check/'+id+'">'+date.toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY)+'</a>');
                }, },
{ data: 'supervision_tagsName', name:'Punto de revisión',width:"auto" },
...
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editsupervision_tags_check/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#supervision_day_check-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        let query = db.collection("cardActivities").where("dayId",">=",last4Weeks.toISODate());//.where("dayId","<=",today.plus({days:1}).toISODate());
        // if(filterName != undefined && filterValue != undefined){
        //     console.log("filter : "+filterName+" == "+filterValue)
        //     query = query.where(filterName,"==",filterValue);
        // }
        console.log("supervision_day_check", "dayId",">=",last4Weeks.toISODate(),"dayId","<=",today.plus({days:1}).toISODate());
        //.orderBy('dayId','desc')
        unsubscribe = query.onSnapshot(async (querySnapshot) => {
            const tmp = [];
            // console.log("supervision_day_check", querySnapshot.size);
            ...
            querySnapshot.forEach((doc) => {
				...
				tempsupervision_tags_check.branchName = "";
				$branches.forEach(elbranches => {
                if(elbranches.id == tempsupervision_tags_check.branch){
                  tempsupervision_tags_check.branchName = elbranches.name;
                }
              });
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Historial de revisiones</h4>

                 {#if filterName != undefined && filterValue != undefined}
                    <FilterName node={profileFilter != null? profileFilter[1]:filterName} value={filterValue} />
                 {/if}
                <!-- <div class="form-group row">
                    <label for="date" class="col-md-4 col-form-label">Ver: </label>
                    <div class="col-md-8"> 
                        <button on:click={()=>view = 0} class="btn {view == 0? 'btn-primary':' btn-secondary'}">Revisiones</button> 
                        <button on:click={()=>view = 1} class="btn {view == 1? 'btn-primary':' btn-secondary'}">Cumplimiento</button></div>
                </div> -->
                <hr />
                {#if view == 0}
                <table id="supervision_day_check-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Fecha</th>
                    ...
                    <tbody>
                        {#if branch != undefined}
                            {#if branch.night == undefined}
                                <h5>Esta estancia no tiene horarios definidos</h5>
                            {:else}
                                {#each Object.entries(checksDates) as cdate}
                                    <tr>
                                        <td>{cdate[1].date.toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY)}</td>
                                        <td>
                                            {#each cdate[1].hours as hour}
                                                <TagsCheck tags={tags} hour={hour.hour} checks={hour.checks} />
                                            {/each}
                                            {#if cdate[1].checks.length > 0}
                                                <div class="col">Fuera de horario: {cdate[1].checks.length}</div>
                                            {/if}
                                        </td>
                                    </tr>
                                {:else}
                                    <h5>Esta estancia no tiene revisiones realizadas</h5> 
                                {/each}
                            {/if}
                        {/if}
                    </tbody>
                ...
                {:else}
                <table id="supervision_day_check-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%; display:block; overflow:auto;">
                    <thead>
                        <tr>
                            <th>Fecha</th>
                            <th>Completado</th>                
                    ...
                    <tbody>
                        {#if branch != undefined}
                                {#each Object.entries(completed) as cdate}
                                    {@const dayId = cdate[1].date.toISODate() }
                                    {#if checksDates[dayId] != undefined}
                                                <tr>
                                                    <td>{cdate[1].date.toLocaleString(DateTime.DATE_MED_WITH_WEEKDAY)}</td>
                                                    <td style="display: flex; flex-direction:row; ">
                                                        <PieMeter total={checksDates[dayId].total} completed={checksDates[dayId].checks} />
                                                        {checksDates[dayId].checks}
                                                        {checksDates[dayId].total}
