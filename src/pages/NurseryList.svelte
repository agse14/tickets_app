...
import NurseryEdit from "./NurseryEdit.svelte";
...
import { crossfade } from 'svelte/transition';
    ...
    if(profileFilter !== null){
        unsubscribeProfile = profile.subscribe(value => {
            ...
            if(!$permissions.admin && !$permissions.nursery){
    ...
    function loadData(){
        ...
        datatable = jQuery('#nursery-dt').DataTable({
            lengthMenu: [
                [ 10, 25, 50, -1 ],
                [ '10 Reporte de enfermería', '25 Reporte de enfermería', '50 Reporte de enfermería', 'Todo' ]
            ],
            data: nursery,
...
{ data: 'patientNameName', name:'Nombre' },
{ data: 'btn_view_week', name:'Semana', render: function (data, type, row, meta) {
                    return '<a href="#'+"/nurseryreport/"+row.id+'" class="btn btn-light waves-effect">Semana</a>';
                }, },
{ data: 'did', name:'Acciones', render: function (data, type, row, meta) {
					...
					if(hasEdit){
						if(modal){
							...
							buttons += '<a href="#/editnursery/'+data+'" class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>';
						}
					}
            ...
            datatable.buttons().container()
                .appendTo('#nursery-dt_wrapper .col-md-6:eq(0)');
    }
	function loadFirebaseData(){
        let query = db.collection("nursery");
        ...
        unsubscribe = query.orderBy('added','desc').onSnapshot(async (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                tempnursery.did = doc.id;
                			
				tempnursery.branchName = "";
				$branches.forEach(element => {
                if(element.id == tempnursery.branch){
                  tempnursery.branchName = element.name;
                }
              });
			
				tempnursery.patientNameName = "";
				$patients.forEach(element => {
                if(element.id == tempnursery.patientName){
                  tempnursery.patientNameName = element.name;
                }
              });
            ...
            if(datatable == undefined){
                ...
                datatable.clear();
                datatable.rows.add(nursery).draw(false);
            }
            // if(datatable != undefined){
...
function deleteDoc(id) {
    Swal.fire({
                ...
                if (result.value) {
                    db.collection("nursery").doc(id).delete();
...
function asyncChange(id, field, newval){
    ...
    var update = {};
    update[field] = newval;
    db.collection("nursery").doc(id).update(update);
}
...
<div class="page-content">
    ...
    <TitleBar title="Reporte de enfermería" base="Inventario" />
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <h4 class="card-title">Administración Reporte de enfermería</h4>
                 {#if filterName != undefined && filterValue != undefined}
                ...
                <div class="row">
                    ...
                    <div class="col-md-auto align-self-end">
                        {#if modal}
                        <button type="button" class="btn btn-primary waves-effect waves-light" on:click={()=>{ setEditId(undefined);}}>Agregar Reportes</button>
                        {:else}
                        <button on:click={function() { 
                            if(filterName != undefined && filterValue != undefined){
                                push('/addnursery/'+filterName+'/'+filterValue);
                            }else{
                                push('/addnursery');
                            }
                            
                        }} class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus font-size-16 align-middle"></i> Agregar Reportes</button>
                        {/if}
                    </div>
                  </div>
                {/if}
                
                <table id="nursery-dt" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            <th>Fecha</th>
<th>Estancia</th>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="editModalScrollableTitle">Agregar Reportes</h5>
                            ...
                            {#if showModal}
                                <NurseryEdit params={editData} onBack={()=>{
                                    editData = {field:filterName, value:filterValue,bid:undefined};
                                    jQuery('#editModalScrollable').modal('hide');
                                    showModal = false;
                                }}/>
...
<!--
    {#each nursery as row (row.id)}
                        <tr in:receive="{{key: row.id}}"
                        out:send="{{key: row.id}}"
                        animate:flip>
                            <td>{row.date}</td>
<td><h5>{row.branchName}</h5></td>
<td><h5>{row.patientNameName}</h5></td>
<td><a use:link={"/nurseryreport/"+row.id} class="btn btn-light waves-effect">Semana</a></td>
                            ...
                            <td>
                                {#if hasEdit}
                                    ...
                                    {:else}
                                     <a href="/editnursery/{row.id}" use:link class="btn btn-light waves-effect"><i class="bx bx-edit-alt font-size-16 align-middle"></i></a>
                                    {/if}
                                {/if}
