...
//   import PatientlogEdit from "./PatientlogEdit.svelte";
  import RequisitionsEdit from "./RequisitionsEdit.svelte";
            ...
            if (recibos[recibo.uid].payroll == "Fijo") {
              ...
              if (recibo.type == 0) {
                ...
                if (typeof recibo.sharedShift === 'object' && recibo.sharedShift !== null) {
                  ...
                  // Iterar sobre las claves (IDs) del objeto sharedShift
                  Object.keys(recibo.sharedShift).forEach(sharedId => {
                      // Buscar el trabajador correspondiente en $workers
                      let sharedWorker = $workers.find(worker => worker.id === sharedId);
                      if (sharedWorker) {
                          recibo.sharedWorkers.push(sharedWorker.name);
                      } else {
                          recibo.sharedWorkers.push(sharedId); // O manejar de otra manera si no se encuentra el trabajador
                      }
                  });
                } else {
                    recibo.sharedWorkers = []; // Manejar caso donde sharedShift no es un objeto válido
                }
                recibos[recibo.uid].total += recibo.amount * recibo.units;
  ...
  $: if (filterBranch !== "All") {
    ...
    totalBranchAmount = total_sem + total_quin + total_esp + total_mant + total_banca;
  }
  ...
  function selDeleteId(eId) {
    // console.log("Paciente eliminar", eId);
    // TODO:checar que el recibo exista con ese UID, si no retrun
  ...
  function deleteDoc(id) {
    Swal.fire({
      ...
      if (result.value) {
        ...
        db.collection("requisitions").doc(id).delete();
  ...
  function updateAmount(id, newAmount) {
    ...
    Swal.fire({
      ...
      if (result) {
        ...
        // Actualiza el registro en la colección 'requisitions' con el nuevo amount
...
<div class="page-content">
  <TitleBar title="Reporte de Requisiciones" base="Dashboard" />
            ...
            {#if filterBranch === "All"}
                ...
                Total de nomina: {filter == "All"
                    ? Intl.NumberFormat("es-MX", {
                        ...
                        total_sem + total_quin + total_esp + total_mant + total_banca,
                    )
                    : filter == "Semanal"
                    ? Intl.NumberFormat("es-MX", {
                        style: "currency",
                ...
                <!-- Agrega este botón donde quieras, por ejemplo, junto a los botones de imprimir y descargar CSV -->
                <div class="col-12 d-flex justify-content-end align-items-end">
                    <button
                        type="button"
                        class="btn btn-success waves-effect"
                        on:click={() => {
                            editData = { type: 1, added: newend, positionsName: " " };
                            showModal = true;
                            jQuery("#editModalScrollable").modal("show");
                        }}
                    >
                        <i class="bx bx-plus font-size-16 align-middle"></i> Agregar nuevo registro
                    </button>
                </div>    
            {/if}
        </div>
...
>
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content" style="overflow-y: scroll;">
      <div class="modal-header">
        {#if deleteData.patientlog != undefined}
      ...
      <table id="tableDel" class="tableDel table-striped">
        ...
        <tbody>
          {#if deleteData.patientlog != undefined}
            {#each deleteData.patientlog as recibo}
              <!-- {#each Object.keys(recibo) as log} -->
              <tr>
                <td>{recibo.branchName}/{recibo.positionsName} 
                ...
                <!-- <td>{recibo.units}</td> -->
                <!-- <td>{recibo.amount.toFixed(2)}</td> -->
                <td>
                  {#if editingIndex === recibo.id} <!-- Usar el 'id' de 'recibo' en lugar de 'index' para evitar problemas -->
                    <!-- Modo de edición: muestra un campo input para 'amount' -->
                    <input
                      type="number"
                    ...
                    />
                  {:else}
                    <!-- Modo de vista: muestra el valor formateado -->
                    <span
                      on:dblclick={() => {
                        editingIndex = recibo.id; // Activa el modo de edición con 'recibo.id'
                      }}
                    >
                      {recibo.amount.toFixed(2)}
                    </span>
          ...
          <tr>
            <td colspan="5">
              <small
                ><b
                  >*Se mostrará la información del turno (Día/Noche) válida, a
                  partir del 15 de Feb 2024</b
                ></small
              >
            </td>
          </tr>
