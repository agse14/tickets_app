<script>
    ...
    function loadData() {
        ...
        datatable = jQuery("#supervision_week-dt").DataTable({
            lengthMenu: [
                [10, 25, 50, -1],
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        return html(
                            buttons +
                                ...
                                data +
                                '\')" class="btn btn-success waves-effect waves-light"><i class="bx bx-check font-size-16 align-middle"></i>OK</button>',
                        );
                    },
                },
            ],
        ...
        datatable
            .buttons()
            .container()
            .appendTo("#supervision_minutes-dt_wrapper .col-md-6:eq(0)");
    }
    function loadFirebaseData() {
        let query = db.collection("supervision_week");
        if (filterName != undefined && filterValue != undefined) {
            console.log("filter : " + filterName + " == " + filterValue);
            query = query.where(filterName, "==", filterValue);
        }

        unsubscribe = query.where("status", "==", 0)
            .orderBy("added", "desc")
            .onSnapshot(async (querySnapshot) => {
                ...
                querySnapshot.forEach((doc) => {
                    ...
                    tempsupervision_obs.statusName =
                        tempsupervision_obs.status != undefined &&
                        supervision_obs_status[tempsupervision_obs.status] !=
                            undefined
                            ? supervision_obs_status[tempsupervision_obs.status]
                            : "";
    ...
    function filterData() {
        if(bid != undefined && bid != ""){
            let filteredData = supervision_obs.filter((item) => {
                return item.branches == bid;
            });
            datatable.clear();
    ...
    function deleteDoc(id) {
        Swal.fire({
            ...
            if (result.value) {
                db.collection("supervision_week").doc(id).delete();
    ...
    function asyncChange(id, field, newval) {
        var update = {};
        update[field] = newval;
        db.collection("supervision_week").doc(id).update(update);
    }

    function markCompleted(id) {
        Swal.fire({
            title: "¿Esta seguro?",
            text: "Desea marcar como completado este registro",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#34c38f",
            cancelButtonColor: "#f46a6a",
            confirmButtonText: "Si, realizado!",
        }).then(function (result) {
            if (result.value) {
                var update = {};
                update["status"] = 1;
...
<div class="page-content">
    <TitleBar title="Minutas de supervisiones" base="Inventario" />

    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h2 class="card-title">Filtro Estancia</h2>
                    <div class="">
                        <p>
