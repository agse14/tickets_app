<script>
  ...
  import { branches } from "../controls/branches.js";
    import { permissions , fbuser } from "../stores";
  import {calculatePercent, concepts} from "../Utils/payments.js";
    ...
    db.collection("patients")
      ...
      .then(async function (query) {
        if (query == null || query == undefined || query.data() == null) {
          ...
          if(patientdata.introcheck && patientdata.introamount > 0){
            values["concepts"].push({
              concept: 1,
              amount: patientdata.introamount,
              canEdit:true,
              canRemove:true
            });	
          }
          ...
          if (patientdata.status == 0) {
            let nextPaymentDesc = dateToPretty(patientdata.nextPayment);
            let monthId = patientdata.nextPayment.startOf("month").toISODate();
            values["concepts"].push({
              concept: 2,
              monthId: monthId,
              datePretty: nextPaymentDesc,
  ...
  function dateToPretty(date) {
    return date.setLocale("es-mx").toLocaleString({ day:"numeric", month: "short" })+" al "+
    date.plus({ months: 1 }).minus({ days: 1 }).setLocale("es-mx").toLocaleString({ day:"numeric", month: "short" });
  }
  ...
  function proccessTableMonths(){
    let monthPaymentsCount = 0;
    for (let index = 0; index < values.concepts.length; index++) {
      if (values.concepts[index].concept == 2) {
        let prettyDate = dateToPretty( patientdata.nextPayment.plus({ months: monthPaymentsCount }));
        values.concepts[index].datePretty = prettyDate;
        values.concepts[index].monthId = patientdata.nextPayment.plus({ months: monthPaymentsCount }).startOf("month").toISODate();
        monthPaymentsCount++;
      }
    }
  }
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="form-group row">
                ...
                >
                <div class="col-md-4">
                  {#if $permissions.admin || $permissions.accounting || $permissions.admin_branch}
                    <PaytypeSelect
                      ...
                      {#if values.concepts != undefined}
                        <tbody>
                          ...
                          options={concepts}
                              ...
                              required
                            />
                          {#if subfield.concept == 2}
                            <small>{subfield.datePretty ?? ""}</small>
                          {/if}
                          </td><td
                                > 
                                {#if subfield.canEdit}
                              ...
                              >
                              <td>
                                {#if subfield.canRemove != false || $permissions.admin || $permissions.accounting}
                                  <button
                                    type="button"
