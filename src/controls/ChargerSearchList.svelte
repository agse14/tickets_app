...
import { users, addUsers } from './users.js';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-charger"+baseId).select2();
        //   jQuery("#select-charger"+baseId).on("select2:select",function(e){
                ...
                var values = {
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} value={valueObject} on:select={handleSelect} on:clear={handleClear}></Select>
                        <!-- <select id="select-charger{baseId}" bind:value={value} class="form-control select2"  >
