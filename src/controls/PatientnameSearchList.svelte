...
import { patients, addPatients } from './patients.js';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-patientName"+baseId).select2();
        //   jQuery("#select-patientName"+baseId).on("select2:select",function(e){
...
const unsubscribe = patients.subscribe(data => {
  ...
  swal({
              ...
              if(name != null){
                var values = {
                  ...
                  // db.collection("patientName")
                  //   .add(values)
                    addPatients(values).then((rid)=>{
                      value = rid;
                      swal({
                    icon: 'success',
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} value={valueObject} on:select={handleSelect} on:clear={handleClear}></Select>
                        <!-- <select id="select-patientName{baseId}" bind:value={value} class="form-control select2"  >
