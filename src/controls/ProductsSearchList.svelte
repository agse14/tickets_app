...
import { products } from './products.js';
 ...
 let rows;
   ...
   onMount(() => {
         // Select2
         if(!inlist){
          jQuery("#select-products"+baseId).select2();
          jQuery("#select-products"+baseId).on("select2:select",function(e){
...
function updateSelect2(){
  if(!inlist){
    setTimeout(function(){
      jQuery("#select-products"+baseId).trigger("change");
                ...
                Swal.fire({
                  ...
                  <div class="{withStyle?'form-group':''}">
                    {#if inlist}
                      ...
                      {/each}
                    {:else}
                        <select id="select-products{baseId}" bind:value={value} class="form-control select2"  >
