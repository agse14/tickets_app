...
import { users, addUsers } from './users';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-users"+baseId).select2();
        //   jQuery("#select-users"+baseId).on("select2:select",function(e){
                ...
                var values = {
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} bind:value={valueObject} on:clear={handleClear}></Select>
                        <!-- <select id="select-users{baseId}" bind:value={value} class="form-control select2"  >
