...
import { workers, addWorkers } from './workers';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-uid"+baseId).select2();
        //   jQuery("#select-uid"+baseId).on("select2:select",function(e){
                ...
                var values = {
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} bind:value={valueObject} on:clear={handleClear}></Select>
                        <!-- <select id="select-uid{baseId}" bind:value={value} class="form-control select2"  >
