...
import { prescriptions, addPrescriptions } from './prescriptions.js';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-prescriptions"+baseId).select2();
        //   jQuery("#select-prescriptions"+baseId).on("select2:select",function(e){
...
const unsubscribe = prescriptions.subscribe(data => {
  ...
  swal({
              ...
              if(name != null){
                var values = {
                  ...
                  //   .add(values)
                    addPrescriptions(values).then((rid)=>{
                      value = rid;
                      swal({
                    icon: 'success',
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} value={valueObject} on:select={handleSelect} on:clear={handleClear}></Select>
                        <!-- <select id="select-prescriptions{baseId}" bind:value={value} class="form-control select2"  >
