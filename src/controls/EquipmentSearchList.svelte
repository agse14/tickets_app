...
import { equipment } from './equipment.js';
 ...
 let rows;
   ...
   onMount(() => {
         // Select2
         if(!inlist){
          jQuery("#select-equipment"+baseId).select2();
          jQuery("#select-equipment"+baseId).on("select2:select",function(e){
...
function updateSelect2(){
  if(!inlist){
    setTimeout(function(){
      jQuery("#select-equipment"+baseId).trigger("change");
    //console.log("onUpdate");
    }, 500);
  }
}

const unsubscribe = equipment.subscribe(data => {
                ...
                Swal.fire({
                  ...
                  <div class="{withStyle?'form-group':''}">
                    {#if inlist}
                      ...
                      {/each}
                    {:else}
                        <select id="select-equipment{baseId}" bind:value={value} class="form-control select2"  >
