...
import { partners } from './partners.js';
 ...
 let rows;
   ...
   onMount(() => {
         // Select2
         if(!inlist){
          jQuery("#select-partner"+baseId).select2();
          jQuery("#select-partner"+baseId).on("select2:select",function(e){
...
function updateSelect2(){
  if(!inlist){
    setTimeout(function(){
      jQuery("#select-partner"+baseId).trigger("change");
    //console.log("onUpdate");
    }, 500);
  }
}

const unsubscribe = partners.subscribe(data => {
                ...
                Swal.fire({
                  ...
                  <div class="{withStyle?'form-group':''}">
                    {#if inlist}
                      ...
                      {/each}
                    {:else}
                        <select id="select-partner{baseId}" bind:value={value} class="form-control select2"  >
