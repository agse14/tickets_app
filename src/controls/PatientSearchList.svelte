...
import { patients, addPatients } from './patients.js';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-patient"+baseId).select2();
        //   jQuery("#select-patient"+baseId).on("select2:select",function(e){
                ...
                var values = {
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} value={valueObject} on:select={handleSelect} on:clear={handleClear}></Select>
                        <!-- <select id="select-patient{baseId}" bind:value={value} class="form-control select2"  >
