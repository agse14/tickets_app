...
import { supervision_tags, addSupervision_tags } from './supervision_tags';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-supervision_tags"+baseId).select2();
        //   jQuery("#select-supervision_tags"+baseId).on("select2:select",function(e){
            
        //     value= e.params.data.id;
        //     if(onSelect != null && typeof onSelect === 'function'){
        //       //console.log(e.params);
        //       $supervision_tags.forEach(element => {
...
const unsubscribe = supervision_tags.subscribe(data => {
  ...
  swal({
              ...
              if(name != null){
                var values = {
                  ...
                  //   .add(values)
                    addSupervision_tags(values).then((rid)=>{
                      value = rid;
                      swal({
                    icon: 'success',
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} bind:value={valueObject} on:clear={handleClear}></Select>
                        <!-- <select id="select-supervision_tags{baseId}" bind:value={value} class="form-control select2"  >
