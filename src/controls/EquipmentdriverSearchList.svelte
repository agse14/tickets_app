...
import { equipmentdriver } from './equipmentdriver.js';
 ...
 let rows;
   ...
   onMount(() => {
         // Select2
         if(!inlist){
          jQuery("#select-equipmentdriver"+baseId).select2();
          jQuery("#select-equipmentdriver"+baseId).on("select2:select",function(e){
            
            value= e.params.data.id;
            if(onSelect != null && typeof onSelect === 'function'){
              //console.log(e.params);
              $equipmentdriver.forEach(element => {
...
function updateSelect2(){
  if(!inlist){
    setTimeout(function(){
      jQuery("#select-equipmentdriver"+baseId).trigger("change");
    //console.log("onUpdate");
    }, 500);
  }
}

const unsubscribe = equipmentdriver.subscribe(data => {
  ...
  Swal.fire({
                ...
                preConfirm: function (name) {
                  var values = {
                    ...
                    return db.collection("equipmentdriver")
                    .add(values);
                ...
                Swal.fire({
                  ...
                  <div class="{withStyle?'form-group':''}">
                    {#if inlist}
                      ...
                      {/each}
                    {:else}
                        <select id="select-equipmentdriver{baseId}" bind:value={value} class="form-control select2"  >
