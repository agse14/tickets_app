...
import { patients, addPatients } from './patients.js';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-patients"+baseId).select2();
        //   jQuery("#select-patients"+baseId).on("select2:select",function(e){
                ...
                var values = {
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} value={valueObject} on:select={handleSelect} on:clear={handleClear}></Select>
                        <!-- <select id="select-patients{baseId}" bind:value={value} class="form-control select2"  >
