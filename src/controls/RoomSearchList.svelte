<script>
  ...
  import { room_inventory, addRoom_inventory } from './room_inventory';
   ...
   let valueObject;
     ...
     onMount(() => {
           // Select2
          //  if(!inlist){
          //   jQuery("#select-room"+baseId).select2();
          //   jQuery("#select-room"+baseId).on("select2:select",function(e){
  ...
  const unsubscribe = room_inventory.subscribe(data => {
    ...
    swal({
                ...
                if(name != null){
                  var values = {
                    ...
                    // db.collection("room")
                    //   .add(values)
                      addRoom_inventory(values).then((rid)=>{
                        value = rid;
                        swal({
                      icon: 'success',
                    ...
                    <div class="{withStyle?'form-group':''}">
                      ...
                      <Select items={rows} bind:value={valueObject} on:clear={handleClear}></Select>
                          <!-- <select id="select-room{baseId}" bind:value={value} class="form-control select2"  >
