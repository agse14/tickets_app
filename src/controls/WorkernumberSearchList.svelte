...
import { workers, addWorkers } from './workers.js';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-workerNumber"+baseId).select2();
        //   jQuery("#select-workerNumber"+baseId).on("select2:select",function(e){
  ...
  swal({
              ...
              if(name != null){
                var values = {
                  ...
                  // db.collection("workerNumber")
                  //   .add(values)
                    addWorkers(values).then((rid)=>{
                      value = rid;
                      swal({
                    icon: 'success',
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} value={valueObject} on:select={handleSelect} on:clear={handleClear}></Select>
                        <!-- <select id="select-workerNumber{baseId}" bind:value={value} class="form-control select2"  >
