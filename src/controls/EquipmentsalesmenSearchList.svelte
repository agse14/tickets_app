...
import { equipmentsalesmen } from './equipmentsalesmen.js';
 ...
 let rows;
   ...
   onMount(() => {
         // Select2
         if(!inlist){
          jQuery("#select-equipmentsalesmen"+baseId).select2();
          jQuery("#select-equipmentsalesmen"+baseId).on("select2:select",function(e){
            
            value= e.params.data.id;
            if(onSelect != null && typeof onSelect === 'function'){
              //console.log(e.params);
              $equipmentsalesmen.forEach(element => {
...
function updateSelect2(){
  if(!inlist){
    setTimeout(function(){
      jQuery("#select-equipmentsalesmen"+baseId).trigger("change");
    //console.log("onUpdate");
    }, 500);
  }
}

const unsubscribe = equipmentsalesmen.subscribe(data => {
  ...
  Swal.fire({
                ...
                preConfirm: function (name) {
                  var values = {
                    ...
                    return db.collection("equipmentsalesmen")
                    .add(values);
                ...
                Swal.fire({
                  ...
                  <div class="{withStyle?'form-group':''}">
                    {#if inlist}
                      ...
                      {/each}
                    {:else}
                        <select id="select-equipmentsalesmen{baseId}" bind:value={value} class="form-control select2"  >
