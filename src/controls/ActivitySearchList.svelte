...
import { activities, addActivities } from './activities.js';
 ...
 let valueObject;
   ...
   onMount(() => {
         // Select2
        //  if(!inlist){
        //   jQuery("#select-activity"+baseId).select2();
        //   jQuery("#select-activity"+baseId).on("select2:select",function(e){
...
const unsubscribe = activities.subscribe(data => {
                ...
                var values = {
                  ...
                  <div class="{withStyle?'form-group':''}">
                    ...
                    <Select items={rows} value={valueObject} on:select={handleSelect} on:clear={handleClear}></Select>
                        <!-- <select id="select-activity{baseId}" bind:value={value} class="form-control select2"  >
